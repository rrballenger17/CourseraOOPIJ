...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		while(i < word.length()){
			...
			if(isVowel(c) ){
				...
				if(i > 0){
					qualify = !isVowel(word.charAt(i-1));
				}
				
				if(qualify){
					
					
					boolean skip = (c == 'e' && i == word.length() - 1 && count > 0);
	...
	public double getFleschScore()
	{
		...
		float syllabals = getNumSyllables();
		
		
		
		
		double score = 206.835 - 1.015 *(words/sentences) - 84.6 * (syllabals/words);
