...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index <= s.length(); index++){
			for(char charCode = 'a'; charCode <= 'z'; charCode++) {
				...
				String copy = s.substring(0, index) +  charCode + s.substring(index);
				
				//list.add(copy);
				
				if(
						!currentList.contains(copy) 
						&& 
						(!wordsOnly||dict.isWord(copy.toString())) 
						&&
						!s.equals(copy.toString())
				) 
				{
					currentList.add(copy.toString());
				}
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index < s.length(); index++){
				...
				String copy = s.substring(0, index) + s.substring(index + 1);
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(queue.size() > 0 && retList.size() < numSuggestions){
			...
			substitution(curr, strings, false);
			insertions(curr, strings, false);
			deletions(curr, strings, false);
